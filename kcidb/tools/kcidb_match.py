...
class PatternValidator:
    ...
    def remove_required_fields(self, schema):
        """We use the same KCIDB schema to validade, but without any required
        fields, so remove the required rule for our validation"""
        ...
        if "$defs" in schema:
            for _, subschema in schema["$defs"].items():
                self.remove_required_fields(subschema)

    def validate_pattern_object(self, pattern_object):
        """Validate if pattern_object from the user is given as expected,
        as per KCIDB schema"""
    ...
    def match_fields(self, pattern_obj, kcidb_obj):
        ...
        Args:
            ...
            kcidb_obj (dict): The KCIDB object containing fields to be matched
            against.
        ...
        def match_item(pattern_item, kcidb_item):
            """Match all sub-fields in pattern_item against those in
            kcidb_item."""
            for sub_key, sub_pattern_value in pattern_item.items():
                if sub_key == "log_regex":  # ignore log_regex here
                    return True
                if sub_key not in kcidb_item:
                    logger.debug("Field %s does not exist "
                                 "in KCIDB entry", sub_key)
                    return False
                if not match_value(sub_pattern_value, kcidb_item[sub_key]):
                    logger.debug("Field %s with value %s does not match %s",
                                 sub_key, kcidb_item[sub_key],
                                 sub_pattern_value)
                    return False
            return True
        ...
        for key, pattern_value in pattern_obj.items():
            ...
            if key not in kcidb_obj:
                logger.debug("Key %s not found in kcidb_obj", key)
                return False
            ...
            if isinstance(pattern_value, list):
                ...
                if not pattern_value or not kcidb_obj[key]:
                    logger.debug("Key %s is empty in one of the objects",
                                 key)
                    return False
    ...
    def check_log_match(self, kcidb_io_object, pattern_object, key):
        """When the pattern_object contains a log_regex, first try to
        match it against log_excerpt, if it can't find any occurrence there,
        it will download the entire log file and try to match on it.
        Returns True if it's able to find the log_regex, False otherwise"""
...
class IncidentGenerator:
    ...
    def create_incident(self, kcidb_io_object, issue_id, issue_version):
        ...
        incident_id = f"{ORIGIN}:{hashlib.sha256(unique_string.encode(
                      )).hexdigest()}"
    ...
    def generate_incidents_from_object(self, kcidb_io_object):
        ...
        for issue in kcidb_io_object['issues']:
            pattern_object = issue['misc']['pattern_object'] \
                if 'pattern_object' in issue['misc'] else {}
            if not pattern_object:
                continue  # Nothing to match
            incidents.extend(self.generate_incident_on_match(
                kcidb_io_object, issue['id'], issue['version'],
                pattern_object))
...
def fetch_kcidb_io_object(query_id, db_conn, query_type):
    command = ["kcidb-query", f"-{query_type[0]}", query_id, "-d",
               db_conn, "--parents"]
...
def validate_kcidb_io_object(kcidb_io_object):
    if not (('tests' in kcidb_io_object and kcidb_io_object['tests']) or
        ...
        logger.error("The KCIDB IO object must contain at "
                     "least one non-empty test or build")
        return False
...
def main():
    ...
    if args.check_test_id or args.check_build_id:
        ...
        kcidb_id = args.check_test_id if args.check_test_id \
            else args.check_build_id
        kcidb_io_object = fetch_kcidb_io_object(
            kcidb_id, args.db_conn, id_type)
        issue_objects = json.load(sys.stdin)
    ...
    if args.ignore_db:
        results = incident_generator.generate_incidents_from_object(
            kcidb_io_object)
    else:
